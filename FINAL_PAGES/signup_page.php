<?php
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if ($username and $first_name and $last_name and $username and $password and $number) {
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
			...
			//Checking if either username or email has already been used
			$query = "select * from user where username='$username' or email='$email'";
			if (mysqli_num_rows(mysqli_query($con,$query)) > 0) {
				echo "<p>Either username or email are taken, please enter different inputs</p>";
			}
			else {
			//Regstering user into database
			//Note that password is salted hashed and encrypted before insertion into database
			...
			echo "<p>Please enter the fields properly</p>";
		}
	}
?>
