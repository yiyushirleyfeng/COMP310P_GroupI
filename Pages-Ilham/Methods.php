<?php
	...
	function generateSearchQuery($category, $ground, $dateStart, $dateEnd) {
		...
		if ($category and $ground and ($dateStart or $dateEnd)) {
			$query = "select * from event join venue on event.venue_id=venue.venue_id where ($categoryString) and ($groundString) and ($dateString) and (number_of_tickets>0) and (curdate()<=deadline_purchase)";
		}
		elseif ($category and $ground) {
			$query = "select * from event join venue on event.venue_id=venue.venue_id where ($categoryString) and ($groundString) and (number_of_tickets>0) and (curdate()<=deadline_purchase)";
		}
		elseif ($category and ($dateStart or $dateEnd)) {
			$query = "select * from event join venue on event.venue_id=venue.venue_id where ($categoryString) and ($dateString) and (number_of_tickets>0) and (curdate()<=deadline_purchase)";
		}
		elseif ($ground and ($dateStart or $dateEnd)) {
			$query = "select * from event join venue on event.venue_id=venue.venue_id where ($groundString) and ($dateString) and (number_of_tickets>0) and (curdate()<=deadline_purchase)";
		}
		elseif ($category) {
			$query = "select * from event join venue on event.venue_id=venue.venue_id where ($categoryString) and (number_of_tickets>0) and (curdate()<=deadline_purchase)";
		}
		elseif ($ground) {
			$query = "select * from event join venue on event.venue_id=venue.venue_id where ($groundString) and (number_of_tickets>0) and (curdate()<=deadline_purchase)";
		}
		elseif ($dateStart or $dateEnd) {
			$query = "select * from event join venue on event.venue_id=venue.venue_id where ($dateString) and (number_of_tickets>0) and (curdate()<=deadline_purchase)";
		}
		else;
		return $query;
	}
